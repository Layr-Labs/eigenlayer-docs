...
:::info
As described [here](../registration/), the ECDSA and BLS keys needed to populate your `.env` can be obtained via the process of registering as an operator for EigenLayer. 
:::
...
EigenDA makes use of a structured reference string (SRS) as part of its KZG verification. Run the following command to download all required files into the `eigenda-operator-setup/resources` directory. 

```
./srs_setup.sh
```

The file is approximately 8GB in size.
